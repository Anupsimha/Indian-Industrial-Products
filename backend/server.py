...
async def list_requirements(
    ...
    for d in docs:
        d_dict = {
            ...
            "quantity": d.quantity, "budget": getattr(d, "budget", None) or "Not specified by the customer",
            "required_by": getattr(d, "required_by", None) or "Not specified by the customer",
        ...
        item = {
            ...
            "product_name": d.product_name,
            "quantity": d.quantity,
            "budget": getattr(d, "budget", None) or "Not specified by the customer",
            "required_by": getattr(d, "required_by", None) or "Not specified by the customer",
            "state": d.state, "city": d.city,
            "industrial_area": d.industrial_area,
...
async def startup():
    ...
    migrations = [
        ...
        "UPDATE enquiries SET budget = 'Not specified by the customer' WHERE budget IS NULL OR budget = '' OR budget = '₹50,000 - 1 Lakh'",
        "UPDATE enquiries SET required_by = 'Not specified by the customer' WHERE required_by IS NULL OR required_by = '' OR required_by = '15 Days'",
