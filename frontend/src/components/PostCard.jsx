...
import { Heart, Bookmark, MessageCircle, MapPin, MoreHorizontal, UserPlus, UserCheck, Send } from "lucide-react";
...
export const PostCard = ({ post, onUpdate }) => {
  ...
  return (
    ...
    >
      ...
      {/* Action bar */}
      <div className="flex items-center justify-between px-3 py-2 lg:px-4 lg:py-3 border-t border-slate-100">
        <div className="flex items-center gap-1">
          <button
            onClick={toggleLike}
            data-testid={`post-like-${post.id}`}
            className="p-2 inline-flex items-center gap-1.5 text-slate-700 hover:text-rose-600"
          >
            <Heart
              size={20}
              className={`${liked ? "fill-rose-600 text-rose-600" : ""} ${popping ? "heart-pop" : ""}`}
            />
            <span className="text-xs lg:text-sm font-semibold">{likes}</span>
          </button>
          <button
            onClick={() => setShowComments((v) => !v)}
            data-testid={`post-comment-${post.id}`}
            className="p-2 inline-flex items-center gap-1.5 text-slate-700 hover:text-blue-800"
          >
            <MessageCircle size={20} />
            <span className="text-xs lg:text-sm font-semibold">{comments.length || post.enquiries_count}</span>
          </button>
          <a
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className="p-2 text-slate-700 hover:text-blue-800 transition-colors"
            aria-label="Save"
          >
            <Bookmark size={20} className={saved ? "fill-blue-800 text-blue-800" : ""} />
          </button>
          <button
            onClick={() => setEnquiryOpen(true)}
            data-testid={`post-enquiry-${post.id}`}
            className="ml-1 px-4 py-1.5 lg:px-5 lg:py-2 rounded-full bg-orange-600 text-white text-xs lg:text-sm font-semibold hover:bg-orange-700 shadow-sm shadow-orange-200 transition-colors"
          >
            Enquiry
          </button>
        </div>
